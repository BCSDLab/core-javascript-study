...
- 비트를 한단위로만 묶으면 검색시간을 줄이고 표현할 수 있는 데이터의 개수도 늘어나지만 낭비도 생김
  ...
  - 사용하지 않을 데이터를 위해 공간을 남겨 놓기 <<<<<< 표현 개수에 제한을 두고 문제가 되지 않을 정도로만 공간을 묶기
...
모든 데이터는 바이트단위의 식별자(메모리 주솟값)을 통해 서로 구분하고 연결한다.
...
> "데이터 변환을 자유롭게, 메모리를 더욱 효율적으로 관리 할 수 있게"
...
- 만일 확보된 공간 내에서만 데이터 변환을 진행할 수 있다면?
  ...
  - 위 작업을 하기 위해서 저장된 데이터를 전부 뒤로 옮기고 이동시킨 주소를 각 식별자에 다시 연결하는 작업을 수행하면 연산이 늘어남
